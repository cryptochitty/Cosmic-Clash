...
const ChoiceDisplay: React.FC<{ choice: Choice | null; label: string; labelColor: string }> = ({ choice, label, labelColor }) => {
    ...
    return (
        <div className="flex flex-col items-center">
            <p className={`mb-4 text-xl font-bold uppercase ${labelColor}`}>{label}</p>
            <div className="w-28 h-28 md:w-36 md:h-36 p-4 bg-black/30 rounded-full border-4 border-gray-600 flex items-center justify-center">
                <div className="w-14 h-14 md:w-20 md:h-20 text-white transition-opacity duration-500">
                    {choice ? icons[choice] : <span className="text-4xl text-gray-500">?</span>}
                </div>
            </div>
        </div>
    );
...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ playerChoice, computerChoice }) => {
    return (
        <div className="flex justify-around items-center my-8">
            ...
            <p className="text-4xl font-bold text-fuchsia-500 mx-4">VS</p>
